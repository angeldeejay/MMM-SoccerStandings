...
```javascript
const stage1 = dedupedFixtures.filter(
	(f) => (f.homeScore !== undefined && f.awayScore !== undefined) || f.live
);
const stage2 = dedupedFixtures.filter(
	(f) => f.date === today && !stage1.includes(f)
);
const stage3 = dedupedFixtures.filter(
	(f) => f.date > today && !stage1.includes(f)
);
```
...
const normalizeTeamForDedup = (team) => {
	return team
		...
		.replace(/&amp;/g, "&")
		.replace(
			/\b(fc|sc|afc|cf|united|city|real|atletico|de|la|ac|inter|sporting|club|madrid|barcelona|munich|dortmund|paris|saint|germain|st)\b/gi,
			""
		)
		.replace(/[^a-z0-9]/g, "")
		.trim();
