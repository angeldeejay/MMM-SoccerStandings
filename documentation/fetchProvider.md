...
- **Split-League Awareness**: For leagues that split into groups (e.g., Romania, Scotland), the module detects if a provider is only returning a single group (like the Championship table). It will automatically escalate to the next provider (usually Wikipedia) to fetch the full multi-group standings.
